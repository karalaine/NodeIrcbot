...
{
     if (_.has(object, key)) // or just (key in obj)
        return [object];
...
{
    var SERVER_URL = "http://data.fmi.fi/fmi-apikey/f4b3c455-56e0-4a21-a4b3-a8c8616ea896/wfs?";
    ...
    var params  = {
        ...
        'place': place,
        'maxlocations': 1,
        'starttime': startTime.toISOString().split(/\.\d+/).join("")
    }
        ...
        if(errors)
        {
            ...
            var xml = xml2js(body, { trim: true }, function(err, result){
                if(!err)
                {
                    ...
                    var measurements = findRecursive(result, "wml2:MeasurementTimeseries"); 
                    var values = {}
                    _.each(measurements, function(m)
                        {
                            var target = m["wml2:MeasurementTimeseries"][0].$['gml:id'].split('-').pop();
                            var value = findRecursive(m, 'wml2:value').pop()['wml2:value'][0];
                            if(value.length > 1 && value != 'NaN')
                                values[target] = value;
                        }
                    )
                    var text = "Sää " + place + " ";
                    if ('t2m' in values)
                        text += 'lämpötila: ' + parseFloat(values['t2m']).toFixed(1) + '°C ';
                    if ('t2m' in values && 'ws_10min' in values)
                    {    // Calculate "feels like" if both temperature and wind speed were found
                        var feels_like = 13.12 + 0.6215 * values['t2m'] - 13.956 * (values['ws_10min'] ** 0.16) + 0.4867 * values['t2m'] * (values['ws_10min'] ** 0.16);
                        text += 'tuntuu kuin: ' + feels_like.toFixed(1) +'°C ';
                    }
                    if ('ws_10min' in values)
                        text += 'tuulen nopeus: ' +  Math.round(values['ws_10min']) +' m/s ';
                    if ('rh' in values)
